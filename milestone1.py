...
class milestone1(QMainWindow):
    ...
    # establishing the connection - helper
    def executeQuery(self, sql_str):
        try:
            conn = psycopg2.connect("dbname ='milestone1db' user='postgres' host='localhost' password='H@ppy123!'" )
        except:
    ...
    def stateChanged(self):
        ...
        # careful with string vals - query wont work without the '
        if self.ui.stateList.currentIndex() >= 0:
            ...
            for i in reversed(range(self.ui.businessTable.rowCount())): #capital r?
            ...
            try:
                ...
                for row in results:
                    for colCount in range(0, len(results[0])):
                        self.ui.businessTable.setItem(currRowCount, colCount, QTableWidgetItem(row[colCount]))
                    currRowCount+=1
            except:
                print("Query failed")
    def cityChanged(self):
        if(self.ui.stateList.currentIndex >= 0) and (len(self.ui.cityList.selectedItems()) > 0):
            ...
            sql_str = (
                        "SELECT name, city, state FROM business WHERE state ='" + state + "' AND city ='" + city + "' ORDER BY name;")
            # print(sql_str)
            results = self.executeQuery(sql_str)
            try:
                results = self.executeQuery(sql_str)
